...
def generateDepolarizingNoiseModel(scaling=1.0):
    nm = noise.NoiseModel()
    noise1 = noise.depolarizing_error(scaling*0.1,1)
    noise2 = noise.depolarizing_error(scaling*0.1/8,2)
    nm.add_all_qubit_quantum_error(noise1,['u1','u2','u3'])
    nm.add_all_qubit_quantum_error(noise2,['cx'])
    return nm

def generateBitflipNoiseModel(scaling=1.0):
    nm = noise.NoiseModel()
    noise1 = noise.pauli_error([('X',0.0125*scaling),('I',1-0.0125*scaling)])
    noise2 = noise.pauli_error([('X',0.025*scaling),('I',1-0.025*scaling)])
    noise2 = noise2.tensor(noise2)
    nm.add_all_qubit_quantum_error(noise1,['u1','u2','u3'])
    nm.add_all_qubit_quantum_error(noise2,['cx'])
    return nm
...
def generateNoiseModel(scaling=1.0):
    ...
    for a,b,c in thermal_errors:
        if a=='cx':
            ...
            n+=1
            if len(c._noise_probabilities)==1:
                tq_prob[0]+= np.asarray(c.probabilities)
            else:
                tq_prob+= np.asarray(c.probabilities)
        elif a=='id':
            if b[0] in [0,'0']:
                id_circ = c._noise_circuits
            if len(c._noise_probabilities)==1:
                id_prob[0]+=c._noise_probabilities[0]
            else:
            ...
            if len(c._noise_probabilities)==1:
                u3_prob[0]+= np.asarray(c._noise_probabilities)
            else:
            ...
            if len(c._noise_probabilities)==1:
                u2_prob[0]+= np.asarray(c._noise_probabilities)
            else:
