...
if act=='2':
    ...
    qubits = input('Select number of qubits: 3, 4\n')
    if qubits in ['3','4']:
        pass
    else:
...
if run in ['noisy']:
    ...
    qs.set_noise_model(custom=True,
            noise_model=nm)
if run in  ['ibm','noisy']:
    # every 50 measurements, recalibrate the measurement filter
    # also, only local measurement errors are being treated
    qs.set_error_mitigation(
            'measure',
            frequency=50,tensored=True,
            mit_pattern=[[i] for i in range(qubits)]
                )
    if not qubits==1:
        qs.set_error_mitigation(
                'ansatz_shift',coeff=1.0,protocol='current')
#
...
if qubits in [1]:
    ...
    if run in ['noisy','ibm']:
        # note tomo_S will not be used in this run
        kwargs = {
                'method':'newton',
                'update':acse_sol,
                'opt_thresh':0.02,
                'S_thresh_rel':0.6,
                'split_ansatz':True,
                'split_threshold':1.0, # >1 -> favoring previous steps
                'S_min':1e-6,
                'convergence_type':'norm',
        ...
        kwargs = {
                ...
                'S_thresh_rel':0.8,
                'split_ansatz':True,
                'split_threshold':1.0, # favors new terms 
                'S_min':1e-6,
                'convergence_type':'norm',
                'use_trust_region':True,
                'processor':proc,
...
while not acse.total.done:
    if run in ['ibm']:
        # depending  on the quality of the device, we need to impose more 
        # severe limits on how many terms are added at each iteration
        print('Replacing A....')
        tI = Line([acse.A],Nq=3)
        nA = Op()
