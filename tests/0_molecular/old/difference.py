...
tomoRe1 = StandardTomography(qs1)
tomoIm1 = StandardTomography(qs1)
tomoRe2 = ReducedTomography(qs2)
tomoIm2 = ReducedTomography(qs2)
...
tomoRe2.generate(real=True,imag=False,mapping='bk',bkSet=bkSet)
tomoIm2.generate(real=False,imag=True,mapping='bk',bkSet=bkSet)
acse = RunACSE(
        st1,qs1,Ins,
        #update='classical',
        update='quantum',
...
acse.build()
acse.run()
diff = st1.rdm.rdm-st2.rdm.rdm
nz =  np.nonzero(diff)
for (i,k,j,l)  in zip(nz[0],nz[1],nz[2],nz[3]):
    if abs(diff[i,k,j,l])>1e-8:
        print(i,k,j,l,diff[i,k,j,l])
